...
from dtpr.utils.functions import save_mpl_canvas
...
def embed_dt2axes(station, faceview, ax=None, bounds_kwargs=None, cells_kwargs=None):
    ...
    if faceview == "phi":
        ...
        ax.set_ylim(0 - height/2 - 10, y0 + height/2 + 10)
        ax.set_xlabel("x [cm]")
        ax.set_ylabel("-z [cm]")
        ax.set_title(r"$\phi$ view")
    elif faceview == "eta":
        ax.set_xlim(z0 - length/2 - 10, z0 + length/2 + 10)
        ax.set_ylim(0 - height/2 - 10, y0 + height/2 + 10)
        ax.set_xlabel("-y [cm]")
        ax.set_ylabel("-z [cm]")
...
def _make_dt_plot(
    ...
    dt_info = DataFrame([particle.to_dict() for particle in ev.filter_particles(particle_type, wh=wh, sc=sc, st=st)])

    if dt_info.empty:
        color_msg(f"No {particle_type} found for the chamber {wh}/{sc}/{st} in the event {ev.iev}", "red")
        dt_info = None
    else:
...
def plot_dt_chamber(
    ...
    with plt.style.context(getattr(style, RUN_CONFIG.dt_plots_configs["mplhep-style"])):
        ...
        _make_dt_plot(ev, wh=wheel, sc=sector, st=station, name=f"dt_plot{tag}_ev{ev.iev}", path=os.path.join(outfolder, "dt_plots"), save=save)
