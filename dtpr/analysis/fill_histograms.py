...
from dtpr.utils.functions import color_msg, error_handler
from dtpr.utils.functions import create_outfolder
from dtpr.utils.config import RUN_CONFIG
from dtpr.utils.functions import flatten
...
def set_histograms_dict():
    ...
    if missing_histos:
        warnings.warn(f"The following histograms could not be found in any of the sources: {', '.join(missing_histos)}")
    ...
    for histo_key, histoinfo in histos_to_fill.items():
        ...
        if hType == "distribution":
            ...
            try:
                ...
                error_handler(type(e), f"Error in function for histogram {histo_key}:" + str(e), exc_traceback=None)
                continue
...
def fill_histos(inpath, outfolder, tag, maxfiles, maxevents, cores):
    """
    ...
    if size > 6:
        ...
        color_msg(f"{displayed_histos} and {size - 6} more...", color="yellow", indentLevel=2)
    else:
        color_msg(f"{', '.join(histograms_to_fill.keys())}", color="yellow", indentLevel=2)
    
    for ev in tqdm(
        ntuple.events[:_maxevents],
        total=_maxevents,
        desc=color_msg(f"Filling histograms", color="purple", indentLevel=1, return_str=True),
        ncols=100,
        ascii=True,
        unit="event"
    ):
