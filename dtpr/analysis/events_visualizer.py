...
class EventsVisualizer(QMainWindow):
    ...
    def populate_event_list(self):
        ...
        for i, event in enumerate(self.ntuple.events[current_count:self.events_loaded]):
            if event is None:
                ...
                item = QListWidgetItem(f"event {event.iev}")
    ...
    def load_more_events(self):
        ...
        self.events_loaded += 100
        self.populate_event_list(self.event_list)
        QApplication.restoreOverrideCursor()
    ...
    def show_local_plot(self, station):
        ...
        window.setWindowTitle(f"Event {event.iev} - Local Plot for {station.name}")
        ...
        for sl in [1, 2, 3]:
            simhits_info = DataFrame([simhit.to_dict() for simhit in event.filter_particles("simhits", wh=wh, sc=sc, st=st, sl=sl)], columns=["l", "w", "particle_type"])
            for row in simhits_info.itertuples():
                ...
                if sl==2:
                    ax2.scatter(center[0], center[1], color=color, s=size)
                else:
                    ax1.scatter(center[0], center[1], color=color, s=size)
        ...
        # for seg in segments:
        #     ax1.axline(xy1=(seg.pos_locx_sl1, sl1_center[1]), xy2=(seg.pos_locx_sl3, sl3_center[1]), color="red", linestyle="--")
...
if __name__ == '__main__':
    input_folder = "../ntuples/DTDPGNtuple_12_4_2_Phase2Concentrator_Simulation_99.root"
    maxfiles = -1
