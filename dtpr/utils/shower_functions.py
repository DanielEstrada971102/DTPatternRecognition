...
def build_fwshowers(ev: Event, threshold=None, debug=False, debug_step=4, debug_path="./results"):
    ...
    digis_df = DataFrame([digi.to_dict() for digi in ev.digis])
    ...
    if debug and ev.iev % debug_step == 0:
    ...
    if debug and ev.iev % debug_step == 0:
        ...
        fig.savefig(f"{debug_path}/fwshower_plots/hits_evolution_ev{ev.iev}.pdf")
        plt.close(fig)
        gc.collect()

    for ish, ((wh, sc, st, sl), (nHits, bx)) in enumerate(shower_info.items()):
        _shower = Shower(iShower=ish, wh=wh, sc=sc, st=st, nDigis=nHits, BX=bx)
        _shower.sl = sl
...
def build_real_showers(ev: Event, threshold=None, debug=False):
    ...
    ev.realshowers = []
    thr = 6 if threshold is None else threshold
    ...
    for wh, sc, st, sl in indexs:
        ...
        simhits_sdf = DataFrame([simhit.to_dict() for simhit in ev.filter_particles("simhits", wh=wh, sc=sc, st=st, sl=sl)])
        digis_sdf = DataFrame([digi.to_dict() for digi in ev.filter_particles("digis", wh=wh, sc=sc, st=st, sl=sl)])
        ...
        if _build_shower:
            _shower = Shower(iShower=sl, wh=wh, sc=sc, st=st) 
            _shower.shower_type=shower_type
            _shower.sl = sl
            ev.realshowers.append(_shower)
            if debug:
                color_msg(
                    f'Realshower detected in (wh, sc, st, sl): ({wh}, {sc}, {st}, {sl}) with only digis',
                    "purple",
                    indentLevel=2,
                )
